...
class GNN_graphpred(nn.Module):
    ...
    def from_pretrained(self, model_file, device='cpu'):
        self.molecule_model.load_state_dict(torch.load(model_file, map_location=device, weights_only=True))
        print("Loaded pretrained model from", model_file)
        return
