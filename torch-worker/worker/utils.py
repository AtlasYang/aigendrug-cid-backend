...
molformer_model = AutoModel.from_pretrained(molformer_model_name, deterministic_eval=True, trust_remote_code=True).to(device)
molformer_tokenizer = AutoTokenizer.from_pretrained(molformer_model_name, trust_remote_code=True)
...
def get_molformer_embedding(smiles_list) -> ndarray:
    embeddings = []
    for smiles in smiles_list:
        inputs = molformer_tokenizer(smiles, return_tensors="pt", padding=True).to(device)
        with torch.no_grad():
...
def get_complex_embedding(smiles_list) -> ndarray:
    ...
    return array([torch.cat((torch.tensor(chemberta_embedding), torch.tensor(fingerprint))).numpy() for chemberta_embedding, fingerprint in zip(chemberta_embeddings, fingerprints)])
...
def get_complex_embedding_molformer(smiles_list) -> ndarray:
    ...
    fingerprints = [get_fingerprint_from_smiles(smiles) for smiles in smiles_list]
    return array([torch.cat((torch.tensor(molformer_embedding), torch.tensor(fingerprint))).numpy() for molformer_embedding, fingerprint in zip(molformer_embeddings, fingerprints)])
...
def get_complex_embedding_chemberta_molformer(smiles_list) -> ndarray:
    ...
    molformer_embeddings = get_molformer_embedding(smiles_list)
    fingerprints = [get_fingerprint_from_smiles(smiles) for smiles in smiles_list]
    return array([torch.cat((torch.tensor(chemberta_embedding), torch.tensor(molformer_embedding), torch.tensor(fingerprint))).numpy() for chemberta_embedding, molformer_embedding, fingerprint in zip(chemberta_embeddings, molformer_embeddings, fingerprints)])